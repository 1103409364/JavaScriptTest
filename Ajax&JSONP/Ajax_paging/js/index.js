...
Administrator.prototype.dictionaryFix = function(dictionary) {
  ...
  dictionary.link =
    "http://talent.baidu.com/external/baidu/index.html#/jobDetail/2/" +
    dictionary.postId;
...
Administrator.prototype.toPage = function(pageNum) {
  ...
  $.get("JSON/getPostList" + pageNum, function(data) {
    ...
    // 职位总数
    let totalPost = _this.postData.rowCount;
    // 总页数发生变化的时候，更新分页导航条
    if (_this.totalPage != totalPost) {
      _this.totalPage = totalPost;
      _this.pagination.update(totalPost);
    }
    // 显示总在招职位数
