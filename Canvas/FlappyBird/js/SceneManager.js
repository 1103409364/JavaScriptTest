  ...
  SceneManager.prototype.render = function () {
    switch (this.sceneNumber) {
      ...
      case 2:
        ...
        game.ctx.drawImage(
          game.R["tutorial"],
          game.canvas.width / 2 - 57,
          game.canvas.height * 0.618
        );
        game.ctx.restore();
        //渲染logo
        game.ctx.drawImage(
      ...
      case 3:
        ...
        for (var i = 0; i < scoreLength; i++) {
          game.ctx.drawImage(
            game.R["shuzi" + game.score.toString().charAt(i)],
            game.canvas.width / 2 - (scoreLength / 2) * 34 + 34 * i,
            game.canvas.height * 0.1
          );
        }
        //渲染小鸟，小鸟应该后渲染，盖住其他对象
        game.bird.render();
      ...
      case 5:
        ...
        //渲染game over
        game.ctx.drawImage(
          game.R["text_game_over"],
          game.canvas.width / 2 - 102,
          game.canvas.height * (1 - 0.618)
        );
    }
  };
  // 进场
  ...
  SceneManager.prototype.bindEvent = function () {
    ...
    if (isMobile()) {
      ...
      game.canvas.addEventListener("touchmove", function (event) {
        event.preventDefault(); // 阻止默认橡皮筋效果
      });
    } else {
    ...
    function clickHandler(mousex, mousey) {
      ...
      switch (_this.sceneNumber) {
        case 1:
          ...
          if (
            mousex > _this.button_playX &&
            mousex < _this.button_playX + 116 &&
            mousey > _this.button_playY &&
            mousey < _this.button_playY + 70
          ) {
            _this.enter(2); //去2号场景
          }
          break;
